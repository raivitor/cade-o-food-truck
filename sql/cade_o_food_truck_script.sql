-- MySQL Script generated by MySQL Workbench and modified by Hiago Miguel
-- Model: CADÊOFOODTRUCK    Version: 0.1
...
CREATE TABLE IF NOT EXISTS `food_truck`.`categoria` (
  ...
  `nome` VARCHAR(255),
  PRIMARY KEY (`id`),
  UNIQUE INDEX `categoria_unq_nome` (`nome` ASC))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `food_truck`.`item_de_cardapio` (
  ...
  INDEX `item_de_cardapio_idx_categoria_id_fk` (`categoria_id` ASC),
  ...
  CONSTRAINT `item_de_cardapio_fk_sabor_id`
    ...
    ON UPDATE CASCADE,
  CONSTRAINT `item_de_cardapio_fk_categoria_id`
    FOREIGN KEY (`categoria_id`)
    REFERENCES `food_truck`.`categoria` (`id`)
    ON UPDATE CASCADE,
